...
|---|---|---|---|
| 1 | [`week-01-video/`](week-01-video/) | **Shifted, Not Changed** — an explainer on Chapter 1's max-subtraction: why softmax subtracts the largest score before exponentiating, what that changes, and where the fix runs out | Submitted |
...
- Measured, the shifted and direct paths differ by `1.1102230246251565e-16` — one machine
  epsilon. Identical in algebra; in floating point merely indistinguishable.
...
and source** — everything a reviewer can read, run and diff. Per
...
python3 -m unittest discover -s evidence   # 28 tests; each names the beat it defends
...
Claude Opus 5 via Claude Code, disclosed **portion by portion** with the method used for
each artifact in [`week-01-video/SOURCES.md`](week-01-video/SOURCES.md) §4, and logged with
dates in [`week-01-video/FRICTIONAL.md`](week-01-video/FRICTIONAL.md). The narration is
synthetic (Kokoro `am_onyx`, local) and disclosed on screen. Not affiliated with or
endorsed by the @NikBearBrown channel.
